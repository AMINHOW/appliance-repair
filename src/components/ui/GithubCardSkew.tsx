...
export default function GithubCardSkew({ className, children }: { className?: string, children: React.ReactNode }) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "flex transform-gpu flex-col gap-4 rounded-3xl border border-border bg-zinc-700 p-10 text-zinc-200 shadow-lg transition-transform ease-linear will-change-transform",
        className,
      )}
      style={{
